...
Backs the Review and Scientific Database (Dataset) pages. Neither page can get
this shape from the generic GET /observations endpoint (schemas/canonical.py
ObservationOut) without either breaking its other caller (the Trajectories page,
which uses plain ObservationOut) or duplicating these joins in two places, so
this is a separate, purpose-built endpoint.
"""
...
from app.schemas.canonical import ObservationReviewOut, ProvenanceRecordOut
...
def list_observations_with_context(
    ...
    for obs, arm, exp, study in rows:
        ...
        out.append(ObservationReviewOut(
...
def get_provenance_image(
    ...
    """
    Serves ProvenanceRecord's own copy of the evidence crop — set once at
    promotion time and never re-derived, so it stays valid even after the
    ExtEvidence row it was copied from is wiped by a later re-extraction.
    """
